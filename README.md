...
### Phase 3 (integrated, opt-in)

Phase 3 now ships as an opt-in control system around training/generation:

- `--phase3` enables controller+adapter decisions at train step boundaries.
- `--phase3-self-improve` enables detached loss-trend advisory LR/clip scaling.
- `--phase3-constraints` enables deterministic repeat-penalty logits shaping for torch generation (independent of `--phase3`).
- `--mode phase3-sim` replays JSONL metric traces through the controller/adapter offline harness.
- `--phase3-trace` uses one shared path for train-time trace output and simulation replay.

Core attractor dynamics remain unchanged; Phase 3 only adjusts runtime controls (learning rate, clipping scale, and optional generation constraints). See [PHASE_3_SPEC.md](PHASE_3_SPEC.md) for architecture and contracts.
...
### Synthetic story orbit sandbox (fast local check)

Use `--dataset synthetic` to run the built-in cyclical toy corpus (no external files).

```bash
.venv/bin/python run_attractor_llm.py --mode train --dataset synthetic \
  --state-dim 512 --hierarchy-levels 2 --dynamics multihead --heads 8 \
  --seq-len 15 --synthetic-vocab-size 50 --synthetic-num-sequences 400 \
  --epochs 8 --lr 0.002 --batch-size 8 \
  --state-clip 5.0 --state-norm-min 0.5 --state-norm-max 3.0
```
...
### Using TinyStories (local archive or extracted JSON)

Use `--dataset tinystories` to train on [TinyStories](https://huggingface.co/datasets/roneneldan/TinyStories). Place `TinyStories_all_data.tar.gz` under `data/tinystories/` or provide extracted JSON files under `data/tinystories/extracted/`. The loader auto-extracts when the local archive exists.
...
Use multinomial sampling for less deterministic continuations:

```bash
.venv/bin/python run_attractor_llm.py "hero quest dragon" \
  --checkpoint checkpoints/attractor_llm_final.pt \
  --sampling multinomial --sample-temperature 0.75 --max-tokens 40
```

### Stabilization controls (Torch convergence)

The torch path now supports explicit state stabilization after each Euler step:

- elementwise clip: `--state-clip` (default `5.0`)
- norm floor: `--state-norm-min` (default `0.5`)
- norm ceiling: `--state-norm-max` (default `3.0`)
- optional final norm projection: `--state-target-norm` (default `0` = disabled)
...
| `--dataset` | `custom` (default), `tinystories` (local archive/JSON), or `synthetic` (generated cyclical stories) |
| `--synthetic-vocab-size` | Synthetic dataset vocabulary width (default **50**) |
| `--synthetic-num-sequences` | Synthetic dataset generated sequence count (default **400**) |
...
| `--state-clip` | Elementwise post-step state clamp (<=0 disables) |
| `--state-norm-min` / `--state-norm-max` | Post-step norm band renormalization range |
| `--state-target-norm` | Optional final norm projection after convergence (<=0 disables) |
| `--sampling` | Torch decode mode: `argmax` (default) or `multinomial` |
| `--sample-temperature` | Temperature for multinomial sampling (default `0.75`) |
| `--train-progress` / `--no-train-progress` | Per-batch tqdm bar during training (default: on; useful on slow CPUs) |
| `--phase3`, `--phase3-self-improve`, `--phase3-budget-*` | Enable bounded train-time Phase 3 control loop |
| `--phase3-constraints`, `--phase3-max-repeat`, `--phase3-repeat-penalty` | Deterministic torch-generation repeat control |
| `--phase3-trace`, `--mode phase3-sim` | Emit and replay Phase 3 JSONL traces |
...
│   ├── datasets.py       # Optional TinyStories (local archive / extracted JSON)
